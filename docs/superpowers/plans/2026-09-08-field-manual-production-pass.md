...
- **`theme.css` keeps its own type scale; the `--m-t-*` ramp does not govern documentation typography.** Spec §1 says the type and spacing scales "replace existing one-off values in `Landing.vue` and `theme.css`". Only `Landing.vue` was converted, and that is where it stops, deliberately.

  The two files are sizing different things. The landing page is a display surface: one `h1` at `clamp(2.75rem, 5.5vw, 4rem)`, a handful of section titles, and a tight set of mono utility labels — a short, closed list of steps that a reader takes in at a glance, which is exactly what a ramp is for. `theme.css` sizes 180 pages of reference prose: a four-level heading hierarchy that has to stay legible when four `h3`s sit under one `h2` on the same screen, callouts and blockquotes that must read as *slightly* quieter than the prose around them rather than a whole ramp step below it, and mono labels in table headers and the outline that answer to line-height and letter-spacing as much as to size. Those sizes were chosen against each other — `h4` above the body it heads, callouts 1.5px under prose, the outline links a half-step under the sidebar — and forcing them onto the landing page's five display steps would round every one of those relationships to the nearest step and lose the reason each was picked.

  So: the `--m-t-*` ramp governs the landing page's display type and the shared utility steps (`--m-t-label`, `--m-t-fine`, `--m-t-micro`, `--m-t-small`), which `theme.css` does consume where the same label voice appears. Docs typography keeps literal sizes, sized for reading. `readability.test.ts` is what holds it honest instead of the ramp: prose at or above the 16px browser default, no heading smaller than the prose it heads, and — added in the final pass — no `font-size` anywhere in `theme.css` below the same 12px legible floor the ramp's utility steps are held to.

  One value does cross between them and so is kept in agreement by hand: `--m-t-body` was raised from `1rem` to `1.0625rem` (17px) to match `.vp-doc p, li`. Its only consumer is the landing page's `.lede`, and a reader who moves from the landing page into the docs should not see body text change size on the way.
